...
of keys and be able to access (mostly delete)
them both by key name and by index.

The algorithm used inserts at the end, but
swaps keys around on deletion.  Hence it is
deterministic and repeatable providing the
sequence of insertion and deletions is also
repeated.  

The system does not store any contents with the keys,
so it is more of a "set" structure, but one that
does not do intersections, unions and the like.

So why would one want this in the first place?
In my case it is for a series of random selections
where keys are extracted from a pool of keys,
and sometimes inserted.  The randomisations should
be the same for the same given PRNG sequence.
